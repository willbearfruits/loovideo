...
const CYCLE: Act[] = [
  ...
  { name: 'STORM', energy: 25, cam: 'drift', daySpeed: 0.01, windBoost: 1.25, set: { 'flora.scatter': 0.95, 'flora.season': 0.45 } },
  ...
  {
    name: 'RETURN', energy: 10, cam: 'leaf', daySpeed: 0.006, season: [0.95, 0.99],
    pulse: ['flora.fell'], dimTo: 0.25, set: { 'flora.vigor': 0.3, 'flora.addFlock': false }
  }
]
...
export class Story {
  ...
  frame(dt: number, audio: AudioFrame, values: Record<string, ParamValue>): void {
    ...
    if (this.acc >= act.energy) this.enter(this.actIdx + 1)
  }
